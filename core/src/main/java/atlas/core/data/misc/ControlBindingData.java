	...
	public static void save(StarMod mod) {
		...
		if(!file.exists()) {
			try {
				...
				AtlasCore.getInstance().logException("An error occurred while creating control bindings file", exception);
			}
		}
		try {
			JSONObject data = new JSONObject();
			...
			AtlasCore.getInstance().logException("An error occurred while saving control bindings file", exception);
		}
	}
	...
	public static void load(StarMod mod) {
		...
		if(file.exists()) {
			try {
				...
				AtlasCore.getInstance().logException("An error occurred while loading control bindings file", exception);
			}
		}
	}
