	...
	public static class AddExchangeBlueprintPanel extends GUIInputPanel {
		...
		public void onInit() {
			...
			if(mode == ExchangeData.SHIPS || mode == ExchangeData.STATIONS) {
				...
				// Name
				nameInput = buildTextBar("Name", 64, 1, contentPane, s -> {
					exchangeData.setName(s);
					return s;
				});
				...
				// Price
				priceInput = buildTextBar("Price (In Bars)", 3, 1, contentPane, s -> {
					try {
						exchangeData.setPrice(Integer.parseInt(s.trim()));
					} catch(NumberFormatException e) {
						exchangeData.setPrice(1);
					}
				...
				// Description
				descriptionInput = buildTextBar("Description", 512, 2, contentPane, s -> {
					exchangeData.setDescription(s);
					return s;
				});
		...
		private GUIActivatableTextBar buildTextBar(String placeholder, int maxLen, int rows, GUIContentPane contentPane, OnInputChangedCallback inputCallback) {
			return new GUIActivatableTextBar(getState(), FontLibrary.FontSize.SMALL, maxLen, rows, placeholder, contentPane.getContent(0), new TextCallback() {
				@Override
