...
public class ExchangeItemScrollableList extends ScrollableTableList<ExchangeData> implements GUIActiveInterface {
    ...
    public void initColumns() {
        if(type == ExchangeData.SHIPS || type == ExchangeData.STATIONS) {
            addColumn(Lng.str("Name"), 15.0F, new Comparator<ExchangeData>() {
                @Override public int compare(ExchangeData o1, ExchangeData o2) { return o1.getName().compareToIgnoreCase(o2.getName()); }
            });
            addColumn(Lng.str("Producer"), 10.0F, new Comparator<ExchangeData>() {
                @Override public int compare(ExchangeData o1, ExchangeData o2) { return o1.getProducer().compareToIgnoreCase(o2.getProducer()); }
            });
            addColumn(Lng.str("Price"), 5.0F, new Comparator<ExchangeData>() {
                @Override public int compare(ExchangeData o1, ExchangeData o2) { return Integer.compare(o1.getPrice(), o2.getPrice()); }
            });
            addColumn(Lng.str("Category"), 10.0F, new Comparator<ExchangeData>() {
                @Override public int compare(ExchangeData o1, ExchangeData o2) { return o1.getCategory().name().compareTo(o2.getCategory().name()); }
            });
            addColumn(Lng.str("Mass"), 5.0F, new Comparator<ExchangeData>() {
                @Override public int compare(ExchangeData o1, ExchangeData o2) { return Float.compare(o1.getMass(), o2.getMass()); }
            });
            addTextFilter(new GUIListFilterText<ExchangeData>() {
                @Override public boolean isOk(String s, ExchangeData item) { return item.getName().toLowerCase().contains(s.toLowerCase()); }
            }, ControllerElement.FilterRowStyle.FULL);
            addTextFilter(new GUIListFilterText<ExchangeData>() {
                @Override public boolean isOk(String s, ExchangeData item) { return item.getProducer().toLowerCase().contains(s.toLowerCase()); }
            ...
            switch(type) {
                case ExchangeData.SHIPS:
                    addDropdownFilter(new GUIListFilterDropdown<ExchangeData, BlueprintClassification>(shipClassifications) {
                        @Override public boolean isOk(BlueprintClassification c, ExchangeData item) { return c == null || item.getClassification() == c; }
            ...
            addColumn(Lng.str("Type"), 5.0f, new Comparator<ExchangeData>() {
                @Override public int compare(ExchangeData o1, ExchangeData o2) { return Short.compare(o1.getItemId(), o2.getItemId()); }
            });
            addColumn(Lng.str("Name"), 15.0F, new Comparator<ExchangeData>() {
                @Override public int compare(ExchangeData o1, ExchangeData o2) { return o1.getName().compareToIgnoreCase(o2.getName()); }
            });
            ...
            addTextFilter(new GUIListFilterText<ExchangeData>() {
                ...
                public boolean isOk(String s, ExchangeData item) {
                    return item.getName().toLowerCase(Locale.ENGLISH).contains(s.toLowerCase(Locale.ENGLISH))
                            || item.getItemInfo().getName().toLowerCase(Locale.ENGLISH).contains(s.toLowerCase(Locale.ENGLISH));
                }
            }, ControllerElement.FilterRowStyle.FULL);
            columnsHeight = 52;
        } else {
    ...
    protected Collection<ExchangeData> getElementList() {
        switch(type) {
            case ExchangeData.SHIPS:    return ExchangeDialog.getShipList();
            case ExchangeData.STATIONS: return ExchangeDialog.getStationList();
            case ExchangeData.ITEMS:    return ExchangeDialog.getItemsList();
            case ExchangeData.WEAPONS:  return ExchangeDialog.getWeaponsList();
    ...
    public void updateListEntries(GUIElementList guiElementList, Set<ExchangeData> set) {
        ...
        for(final ExchangeData data : set) {
            if(type == ExchangeData.SHIPS || type == ExchangeData.STATIONS) {
                ...
                GUIAncor anchor = new GUIAncor(getState(), pane.getWidth() - 28.0f, 28.0f) {
                    @Override public void draw() { super.draw(); setWidth(pane.getWidth() - 28.0f); }
                };
    ...
    private GUIHorizontalButtonTablePane redrawButtonPane(final ExchangeData data, GUIAncor anchor) {
        boolean isOwner = GameClient.getClientPlayerState().getName().equals(data.getProducer())
                && (type == ExchangeData.SHIPS || type == ExchangeData.STATIONS);
        ...
        if(isOwner || GameClient.getClientPlayerState().isAdmin()) {
            buttonPane.addButton(0, 0, Lng.str("REMOVE"), GUIHorizontalArea.HButtonColor.RED, new GUICallback() {
                ...
                @Override public boolean isActive(InputState inputState) { return true; }
            });
        } else {
    ...
    private void buyItem(ExchangeData data) {
        ...
        if(type == ExchangeData.WEAPONS) {
            PacketUtil.sendPacket(GameClient.getClientPlayerState(),
                    new PlayerActionCommandPacket(AtlasExchange.GIVE_ITEM,
                            GameClient.getClientPlayerState().getName(),
                            String.valueOf(data.getItemId()), "1", "true"));
            if(goldBarId != -1) InventoryUtils.consumeItems(playerInventory, goldBarId, data.getPrice());
        } else {
            PacketUtil.sendPacket(GameClient.getClientPlayerState(),
                    new PlayerActionCommandPacket(AtlasExchange.GIVE_ITEM,
                            GameClient.getClientPlayerState().getName(),
                            String.valueOf(data.getItemId()), String.valueOf(data.getItemCount()), "false"));
        }
    }
    ...
    private void buyBlueprint(ExchangeData data) {
        ...
        req.toSaveShip     = -1;
        req.directBuy      = true;
        ((GameClientState) getState()).getPlayer().getNetworkObject()
                .catalogPlayerHandleBuffer.add(new RemoteBlueprintPlayerRequest(req, false));
        short goldBarId = getGoldBarId();
        if(goldBarId != -1)
        ...
        if(AtlasExchange.ADD_BARS != -1) {
            PacketUtil.sendPacket(GameClient.getClientPlayerState(),
                    new PlayerActionCommandPacket(AtlasExchange.ADD_BARS,
                            GameClient.getClientPlayerState().getName(),
                            data.getProducer(),
    ...
    private static boolean isInBuildSector(GameClientState state) {
        ...
        try {
            ...
            return (boolean) cls.getMethod("isPlayerInAnyBuildSector",
                    org.schema.game.common.data.player.PlayerState.class).invoke(manager, state.getPlayer());
        } catch(Exception e) {
            return false;
        }
    }
